...
int main(int argc, char *argv[])
{
	while (1) {
          	...
          	//User Input
          	command.input = (char *) malloc(512);
          	command.copy = (char *) malloc(512);
          	command.max = 512;
		...
		int retval;
	  	...
	  	printf("sshell$ ");
          	fflush(stdout);
          	fgets(command.input, command.max, stdin);
		...
		strtok(command.input, "\n");
	        
		/***** PARSING ARGS *******/
		// if &
		if (strstr(command.input, "&") && (strlen(command.input) == 1)) {
			...
			//Split command into tokens
	  		command = splitCommand(command, command.input);
	 			...
	 			if (command.inputRedir == true) {
							...
							pid = fork(); 
                                        		if (pid == 0) {
                                                		...
                                                		waitpid(-1, &retval, 0);
                                                		fprintf(stderr, "+ completed '%s' [%d]\n", command.input, WEXITSTATUS(retval));
                                        		} else {
                                                		perror("fork");
								exit(1);
                                        		}
...
void pipeCommand(struct Command command) { 
	...
	command = splitPipe(command, command.input);
	...
	if (strstr(command.pipeCommands[0], ">")) {
		...
		while (command.pipeCommands[pipeIndex] != NULL) {
  		...
  		fprintf(stderr, "+ completed '%s'", command.input);
